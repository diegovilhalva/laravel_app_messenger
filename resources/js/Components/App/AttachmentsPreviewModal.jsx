...
const AttachmentsPreviewModal = ({ attachments, index, show = false, onClose = () => { } }) => {
  const [currentIndex, setCurrentIndex] = useState(0)
  const attachment = useMemo(() => {
    return attachments[currentIndex]
  }, [attachments, currentIndex])
  const previewableAttachments = useMemo(() => {
    return attachments.filter((attachment) => isPriviwable(attachment))
  },[attachments])
                    ...
                    }
                    {
                      attachment && (
                        <div className="flex items-center justify-center w-full h-full p-3">
                          {
                            isImage(attachment) && (
                              <img src={attachment.url} className="mex-w-full max-h-full" />
                            )
                          }
                          {
                            isVideo(attachment) && (

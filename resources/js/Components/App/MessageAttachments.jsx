...
import { isAudio, isImage, isPDF, isPriviwable, isVideo } from "@/helpers"
            ...
            className={`group flex flex-col items-center justify-center text-gray-500 relative cursor-pointer ` + (isAudio(attachment) ? 'w-84' : 'w-32 aspect-square bg-blue-100')}>
              {
                !isAudio(attachment) && (
                  <a
                    ...
                    href={attachment.url} className="z-20 opacity-100 group-hover:opacity-100 transition-all w-8 h-8 flex items-center justify-center text-gray-100 bg-gray-700
                  rounded absolute right-0 top-0 cursor-pointer hover:bg-gray-800" download
                  >
                    <ArrowDownTrayIcon />
                  </a>
                )
